...
async def upload_share(share: ShareRequest, file: UploadFile = File(), db: Session = Depends(get_db), current_user: User = Depends(auth_service.get_current_user)):
    ...
    try:
        ...
        result = upload_image(file.file, folder="photo_base",width=360, height=480, crop="fill")
        
        url, _ = cloudinary_url(result["public_id"], format=result["format"])
        
        db_share = await repository_shares.create_share(share, url, db, current_user)
        return db_share
...
@router.post('/qr', response_model=List[ShareResponce], status_code=status.HTTP_201_CREATED) 
...
@router.get('/myshares', response_model=List[ShareResponce])
